...
export default function Form() {
  const [formData, setFormData] = useState({
    senderName: "",
    senderEmail: "", // New field for sender email
    recipientName: "",
    email: "",
    extra: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    data.set("senderEmail", formData.senderEmail); // Set the sender email in the FormData
      ...
      <div className="grid gap-y-12 md:grid-cols-1 md:gap-x-12 ">
        <form onSubmit={handleSubmit} className="grid gap-y-5 text-amber-700 mx-2">
          ...
          <div className="flex flex-col">
            ...
            />
            <div className="text-slate-400 text-right py-2">
              {characterCount} / 200
            </div>
          </div>
          <button
